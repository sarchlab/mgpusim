...
__kernel void MaxPoolForward(int nthreads, __global float* bottom_data, int num,
  ...
  for (int h = hstart; h < hend; h++) {
    // __asm__ volatile("v_log_legacy_f32 %0, %1" : "=v"(hstart) : "v"(hstart));
    // __asm__ volatile("v_log_legacy_f32 %0, %1" : "=v"(h) : "v"(h));
    // __asm__ volatile("v_log_legacy_f32 %0, %1" : "=v"(hend) : "v"(hend));

    // __asm__ volatile("v_log_legacy_f32 %0, %1" : "=v"(wstart) : "v"(wstart));
    // __asm__ volatile("v_log_legacy_f32 %0, %1" : "=v"(wend) : "v"(wend));

    for (int w = wstart; w < wend; w++) {
      // __asm__ volatile("v_log_legacy_f32 %0, %1" : "=v"(w) : "v"(w));
