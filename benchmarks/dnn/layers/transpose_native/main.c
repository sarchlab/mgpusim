...
int main(int argc, char *argv[]) {
  ...
  unsigned int output_size[4] = {4, 2, 3, 3};
  unsigned int order[4] = {1, 0, 2, 3};
  float h_input[] = {
  ...
  kernel = clCreateKernel(program, "transpose_tensor", &err);
  if (err != CL_SUCCESS) {
  ...
  d_out_size = clCreateBuffer(context, CL_MEM_READ_ONLY, dim * sizeof(uint32_t),
                              NULL, NULL);
  d_order = clCreateBuffer(context, CL_MEM_READ_ONLY, dim * sizeof(uint32_t),
                           NULL, NULL);
  d_order = clCreateBuffer(context, CL_MEM_READ_ONLY, dim * sizeof(uint32_t),
                           NULL, NULL);
  d_in_index_buf =
  ...
  err |= clEnqueueWriteBuffer(queue, d_order, CL_TRUE, 0,
                              sizeof(order), order, 0, NULL, NULL);
  if (err != CL_SUCCESS) {
  ...
  err |= clSetKernelArg(kernel, 4, sizeof(cl_mem), &d_order);
