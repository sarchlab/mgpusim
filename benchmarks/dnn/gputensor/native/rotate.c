...
int main(int argc, char *argv[]) {
  ...
  unsigned int output_size[4] = {2, 4, 3, 3};
  float h_input[] = {
  ...
  kernel = clCreateKernel(program, "rotate_tensor", &err);
  if (err != CL_SUCCESS) {
  ...
  err |= clSetKernelArg(kernel, 4, sizeof(cl_mem), &d_in_index_buf);
  err |= clSetKernelArg(kernel, 5, sizeof(cl_mem), &d_out_index_buf);
  err |= clSetKernelArg(kernel, 6, sizeof(cl_int), &dim);
