...
int main(int argc, char *argv[]) {
  unsigned int dim = 4;
  unsigned int num_element = 72;
  unsigned int num_out_element = 200;
  unsigned int input_size[4] = {2, 4, 3, 3};
  unsigned int output_size[4] = {2, 4, 5, 5};
  unsigned int dilate[2] = {2, 2};
  float h_input[] = {
  ...
  d_output =
      ...
      num_out_element * sizeof(float), NULL, NULL);
  ...
  d_out_size = clCreateBuffer(context, CL_MEM_READ_ONLY, dim * sizeof(uint32_t),
                              NULL, NULL);
  d_dilate = clCreateBuffer(context, CL_MEM_READ_ONLY, 2 * sizeof(uint32_t),
                           NULL, NULL);
  d_in_index_buf =
      clCreateBuffer(context, CL_MEM_READ_WRITE,
                     dim * num_out_element * sizeof(int32_t), NULL, NULL);
  int *h_in_index_buf = malloc(dim * num_out_element * sizeof(int32_t));
  d_out_index_buf =
      clCreateBuffer(context, CL_MEM_READ_WRITE,
                     dim * num_out_element * sizeof(int32_t), NULL, NULL);
  int *h_out_index_buf = malloc(dim * num_out_element * sizeof(int32_t));
  float *h_out = malloc(num_out_element * sizeof(float));
  ...
  clEnqueueReadBuffer(queue, d_out_index_buf, CL_TRUE, 0,
                      dim * num_out_element * sizeof(int), h_out_index_buf,
                      0, NULL, NULL);
  clEnqueueReadBuffer(queue, d_in_index_buf, CL_TRUE, 0,
                      dim * num_out_element * sizeof(int), h_in_index_buf,
                      0, NULL, NULL);
  ...
  printf("\n\nIndex:\n");
  for (int r = 0; r < num_out_element; r++) {
    printf("%d: ", r);
