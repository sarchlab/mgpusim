...
int main(int argc, char *argv[]) {
  unsigned int dim = 4;
  unsigned int num_in_element = 72;
  unsigned int num_out_element = 18;
  unsigned int input_size[4] = {2, 4, 3, 3};
  unsigned int output_size[3] = {2, 3, 3};
  float h_input[] = {
  ...
  size_t global_size = 18 * local_size;
  ...
  kernel = clCreateKernel(program, "sum_one_axis", &err);
  if (err != CL_SUCCESS) {
  ...
  d_in_index_buf =
      clCreateBuffer(context, CL_MEM_READ_WRITE,
                     global_size * dim * sizeof(int32_t), NULL, NULL);
  int *h_in_index_buf = malloc(global_size * dim * sizeof(int32_t));
  d_out_index_buf =
      clCreateBuffer(context, CL_MEM_READ_WRITE,
                     global_size * dim  * sizeof(int32_t), NULL, NULL);
  int *h_out_index_buf = malloc(global_size * dim  * sizeof(int32_t));
  float *h_out = malloc(global_size * sizeof(float));
  ...
  err |= clSetKernelArg(kernel, 4, sizeof(cl_int), &in_dim);
  err |= clSetKernelArg(kernel, 5, sizeof(cl_int), &axis);
  err |= clSetKernelArg(kernel, 6, sizeof(cl_mem), &d_in_index_buf);
  err |= clSetKernelArg(kernel, 7, sizeof(cl_mem), &d_out_index_buf);
  err |= clSetKernelArg(kernel, 8, local_size * sizeof(float), NULL);
  ...
  clEnqueueReadBuffer(queue, d_out_index_buf, CL_TRUE, 0,
                      global_size * dim * sizeof(int), h_out_index_buf,
                      0, NULL, NULL);
  clEnqueueReadBuffer(queue, d_in_index_buf, CL_TRUE, 0,
                      global_size * dim * sizeof(int), h_in_index_buf,
                      0, NULL, NULL);
  clEnqueueReadBuffer(queue, d_output, CL_TRUE, 0,
                      num_out_element * sizeof(float), h_out,
                      0, NULL, NULL);
  err = clFinish(queue);
  if (err != CL_SUCCESS) {
