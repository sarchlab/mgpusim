...
MGPUSim is a cycle-accurate GPU simulator modeling AMD CDNA3 instruction set architecture. It uses the Akita discrete-event simulation framework (`github.com/sarchlab/akita/v4`).

### Two Simulation Modes

1. **Emulation Mode** (`amd/emu/`): Fast functional simulation
   - `ComputeUnit` executes wavefronts instruction-by-instruction
   - `ALUImpl` implements all CDNA3 instruction semantics (VALU, SALU, LDS, FLAT, etc.)
...
- **`amd/insts/`**: Instruction definitions, decoder, disassembler, HSACO parsing
...
- **V2/V3**: 256-byte header per kernel in `.text` section (legacy format)
...
- Platform configuration in `samples/*/platform.go`, `mi300a.go`, `shaderarray.go`
