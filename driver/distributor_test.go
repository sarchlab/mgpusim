...
var _ = ginkgo.Describe("Distributor", func() {
	...
	ginkgo.It("should distribute pages", func() {
		memAllocator.EXPECT().
			Remap(ctx, uint64(0x100000000), uint64(0x1000), 1)
		memAllocator.EXPECT().
			Remap(ctx, uint64(0x100001000), uint64(0x1000), 2)
		memAllocator.EXPECT().
			Remap(ctx, uint64(0x100002000), uint64(0x1000), 3)
		memAllocator.EXPECT().
			Remap(ctx, uint64(0x100003000), uint64(0x1000), 3)
		memAllocator.EXPECT().
			Remap(ctx, uint64(0x100004000), uint64(0x1000), 3)
