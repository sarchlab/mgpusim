#!/usr/bin/env python3
"""Archived MI300A terminal-discovery workflow helper.

This helper keeps the archived workflow YAML small while preserving the
historical per-benchmark terminal-discovery shard contract:
...
EXPECTED_BRANCH = "archived-full-run-contract"
EXPECTED_ATTEMPTS = 1416
EXPECTED_ROWS = 81
EXPECTED_SHARDS = 6
EXPECTED_TIMEOUT_SEC = 3600
ARTIFACT_PREFIX = "archived-full-run-outcomes"
...
def emit_matrix(_args: argparse.Namespace) -> int:
    ...
    lines = [
        "# Archived MI300A terminal-discovery matrix validation",
        "",
    ...
    summary = "\n".join(lines) + "\n"
    Path("archived_full_run_matrix_summary.md").write_text(summary, encoding="utf-8")
    print(summary)
    return 0
...
def outcome_base(row: Mapping[str, Any], attempt: Mapping[str, Any], output_dir: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "terminal": True,
        "source_label": "archived_full_run_contract",
        "branch": branch,
        "head_branch": branch,
        "ref": branch,
        "head_sha": head_sha,
        ...
        "dispatch_workflow": ".github/workflows/benchmark.yml",
        "workflow_name": "MI300A Benchmark / archived full-run contract",
        "run_id": int(run_id) if run_id and run_id.isdigit() else run_id,
        "run_database_id": int(run_id) if run_id and run_id.isdigit() else run_id,
...
def run_attempt(row: Mapping[str, Any], attempt: Mapping[str, Any], binary: Path, output_dir: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix=f"archived-full-run-{benchmark}-") as tmp:
        work_dir = Path(tmp)
        ...
        if kernel_time_ms is None:
            ...
            return finalize_outcome(
                ...
                detail="sqlite metrics file did not contain kernel_time",
                exit_code=exit_code,
                started_at=started_at,
            )
...
def summarize_and_write(row: Mapping[str, Any], output_dir: Path, outcomes: Sequence[Mapping[str, Any]]) -> None:
    ...
    container = {
        ...
        "source_label": "archived_full_run_contract",
        "branch": os.environ.get("GITHUB_REF_NAME") or EXPECTED_BRANCH,
        "head_branch": os.environ.get("GITHUB_REF_NAME") or EXPECTED_BRANCH,
        "ref": os.environ.get("GITHUB_REF_NAME") or EXPECTED_BRANCH,
        "head_sha": os.environ.get("GITHUB_SHA"),
    ...
    lines = [
        f"# Archived terminal discovery row: {row.get('benchmark')}",
        "",
...
def count_outcomes(args: argparse.Namespace) -> int:
    ...
    sources = collector.collect_outcome_sources([root if root.is_absolute() else REPO_ROOT / root for root in roots])
    plan_indices = [int(source.outcome.get("plan_index")) for source in sources]
    unique = set(plan_indices)
    duplicates = sorted({idx for idx in unique if plan_indices.count(idx) > 1})
    missing = sorted(set(range(1, EXPECTED_ATTEMPTS + 1)) - unique)
    extra = sorted(unique - set(range(1, EXPECTED_ATTEMPTS + 1)))
    report = {
        "record_count": len(sources),
        "unique_plan_index_count": len(unique),
        "duplicate_plan_indices": duplicates[:20],
        "missing_plan_indices": missing[:20],
        "extra_plan_indices": extra[:20],
        "complete": len(sources) == EXPECTED_ATTEMPTS and len(unique) == EXPECTED_ATTEMPTS and not duplicates and not missing and not extra,
    }
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Archived terminal-discovery full-run contract helper")
