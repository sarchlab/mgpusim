// ga.cpp -- Heteromark Genetic Algorithm benchmark (HIP)
...
int main(int argc, char** argv) {
    ...
    HIP_CHECK(hipMalloc(&d_population, popBytes));
    HIP_CHECK(hipMalloc(&d_selected,   popBytes));
    HIP_CHECK(hipMalloc(&d_fitness,    fitnessBytes));
    HIP_CHECK(hipMalloc(&d_target,     targetLen * sizeof(char)));

    HIP_CHECK(hipMemcpy(d_target, target, targetLen * sizeof(char),
    ...
    BenchResult r = runBenchmark("ga", size_str, iters, [&]() {
        ...
        HIP_CHECK(hipMemcpy(d_population, h_population, popBytes,
                            hipMemcpyHostToDevice));
        ...
        for (int gen = 0; gen < numGenerations; ++gen) {
            ...
            HIP_CHECK(hipMemcpy(d_population, d_selected, popBytes,
    ...
    HIP_CHECK(hipMemcpy(h_fitness, d_fitness, fitnessBytes,
