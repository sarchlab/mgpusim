...
Reference CSV format (e.g., workloads/results/kernel_timings_20260317-075319-odyssey.csv; note: old mi300a.csv/mi300a_120cu.csv have been removed):
...
# ---------------------------------------------------------------------------
# Kernel-name normalisation helpers
# ---------------------------------------------------------------------------
# Canonical mapping: maps known variant names to a single normalised form.
# All keys are **lowercase**.  Values are the normalised (canonical) name that
# will be used for matching – also lowercase so that the matching is fully
# case-insensitive.
# ---------------------------------------------------------------------------
# Microbenchmark exclusion set
# ---------------------------------------------------------------------------
# These 6 kernels are synthetic microbenchmarks whose HW execution times fall
# in the launch-overhead / noise region.  They are excluded from the app-only
# MAPE calculation because they don't represent real application behaviour.
MICROBENCHMARKS = {
    'sfun_sin', 'int_mad', 'branch_div_50pct',
    'mem_latency_chase', 'busspeeddownload', 'busspeedreadback',
}

_KERNEL_NAME_ALIASES: Dict[str, str] = {
    # Polybench CamelCase → lowercase
    "2dconvolution": "2dconvolution",
    "3dconvolution": "3dconvolution",
    # Parboil: CI directory name → HW/sim kernel name
    ...
    "lbm_stream_collide": "lbm_stream_collide",
    ...
    "stencil_kernel": "stencil_kernel",
    "parboil_tpacf": "tpacf_dd",
    "tpacf_dd": "tpacf_dd",
    # SHOC
    "shoc_gemm": "gemm",
    # SpMV variants
    "spmv_csr_scalar": "spmv_csr",
    "spmv_csr": "spmv_csr",
    # Rodinia
    "backprop": "layerforward",
    "layerforward": "layerforward",
    "streamcluster": "streamcluster_pgain",
    "streamcluster_pgain": "streamcluster_pgain",
    # FFT
    "fft1d_512": "fft1d_512",
    # Heteromark: bitonicsort → sort  EXCLUDED — algorithm mismatch
    # HW uses radix sort, sim uses bitonic sort. See Otto's analysis.
    # 'bitonicsort': 'sort',
    # 'sort': 'sort',
    # Lonestar: sim kernel names → HW benchmark names
    'compute_forces_kernel': 'bh',
    'update_bodies_kernel': 'bh',
    'sssp_relax_kernel': 'sssp',
    'identify_bad_triangles': 'dmr',
    'refine_triangles': 'dmr',
    'bh': 'bh',
    'sssp': 'sssp',
    'dmr': 'dmr',
}
...
def _normalise_problem_size(kernel: str, ps: str) -> str:
    """Normalise a problem_size string so that simulator and reference formats
    match.

    Known transformations (applied after lowercasing the *kernel* name):
      - nw: strip 'length=' prefix        ('length=64'  -> '64')
      - bfs: strip 'nodes' prefix          ('nodes1024'  -> '1024')
      - pagerank: extract leading number    ('nodes64_sparsity0.5_priters2' -> '64')
      - kmeans: extract leading number      ('pts1024_feat8_clus5'          -> '1024')
      - floydwarshall: strip '_nodes' suffix ('8_nodes'  -> '8')
    """
    import re

    kern = kernel.strip().lower()
    ps = ps.strip()

    if kern == "nw":
        # 'length=64' -> '64'
        if ps.startswith("length="):
            return ps[len("length="):]
        return ps

    if kern == "bfs":
        # 'nodes1024' -> '1024'
        if ps.startswith("nodes"):
            return ps[len("nodes"):]
        return ps

    if kern == "pagerank":
        # 'nodes64_sparsity0.5_priters2' -> '64'
        m = re.match(r"nodes(\d+)", ps)
        if m:
            return m.group(1)
        return ps

    if kern == "kmeans":
        # 'pts1024_feat8_clus5' -> '1024'
        m = re.match(r"pts(\d+)", ps)
        if m:
            return m.group(1)
        return ps

    if kern == "floydwarshall":
        # '8_nodes' -> '8'
        if ps.endswith("_nodes"):
            return ps[: -len("_nodes")]
        return ps
...
def _normalise_kernel_name(name: str) -> str:
    """Return a normalised, lowercase kernel name for matching purposes.

    Applies ``_KERNEL_NAME_ALIASES`` after lower-casing.  If the name is not
    in the alias table the lowercased form is returned unchanged.
    """
    lower = name.strip().lower()
    return _KERNEL_NAME_ALIASES.get(lower, lower)
...
def read_reference_csv(filepath: str) -> Dict[Tuple[str, str], float]:
    """Read reference CSV and return dict of (normalised_kernel, size) -> avg_ms.

    Kernel names are normalised to lowercase with alias resolution so that
    matching against simulator data is case-insensitive.
    ...
    with open(filepath, "r") as f:
        ...
        for row in reader:
            norm_kernel = _normalise_kernel_name(row["kernel_name"])
            norm_size = _normalise_problem_size(norm_kernel, row["problem_size"])
            key = (norm_kernel, norm_size)
...
def read_simulator_csv(filepath: str) -> Dict[Tuple[str, str], float]:
    ...
    Kernel names are normalised (lowercased + alias-resolved) so that
    matching against reference data is case-insensitive.
    """
    data = {}
    with open(filepath, "r") as f:
        reader = csv.DictReader(f)
        ...
        for row in reader:
            kernel = _normalise_kernel_name(row["kernel_name"])
            size = _normalise_problem_size(kernel, row["problem_size"])
            key = (kernel, size)

            if "sim_time_ms" in fieldnames:
                sim_ms = float(row["sim_time_ms"])
            elif "sim_ms" in fieldnames:
                val = row["sim_ms"].strip()
                if not val:
                    continue
                sim_ms = float(val)
            elif "avg_ms" in fieldnames:
                val = row["avg_ms"].strip()
                if not val:
                    continue
                sim_ms = float(val)
...
def print_comparison_results(
    ...
    if matched_points:
        ...
        # --- App-only statistics (excluding microbenchmarks) ---
        app_matched = [
            p for p in matched_points
            if p.kernel_name not in MICROBENCHMARKS
        ]
        if app_matched:
            app_errors = [p.rel_error for p in app_matched if p.rel_error is not None]
            app_abs_errors = [abs(e) for e in app_errors]
            app_avg = sum(app_abs_errors) / len(app_abs_errors)
            app_median = sorted(app_abs_errors)[len(app_abs_errors) // 2]
            app_within_10 = sum(1 for e in app_abs_errors if e <= 0.10) / len(app_abs_errors)
            app_within_25 = sum(1 for e in app_abs_errors if e <= 0.25) / len(app_abs_errors)
            app_within_50 = sum(1 for e in app_abs_errors if e <= 0.50) / len(app_abs_errors)

            # App-only WMAPE
            app_non_noise = [
                p for p in app_matched
                if p.real_ms >= 0.01 and p.sim_ms is not None
            ]
            if app_non_noise:
                app_wmape_num = sum(abs(p.sim_ms - p.real_ms) for p in app_non_noise)
                app_wmape_den = sum(p.real_ms for p in app_non_noise)
                app_wmape = app_wmape_num / app_wmape_den if app_wmape_den > 0 else 0.0
            else:
                app_wmape = 0.0

            print("\n" + "=" * 110)
            print("APP-ONLY STATISTICS (excluding microbenchmarks)")
            print("=" * 110)
            print(f"  Excluded microbenchmarks:{', '.join(sorted(MICROBENCHMARKS))}")
            print(f"  App matched points:      {len(app_matched)}")
            print(f"  Average |relative error|:{app_avg:.1%}")
            print(f"  Median |relative error|: {app_median:.1%}")
            print(f"  Within 10% error:        {app_within_10:.1%}")
            print(f"  Within 25% error:        {app_within_25:.1%}")
            print(f"  Within 50% error:        {app_within_50:.1%}")
            print(f"  Weighted MAPE:           {app_wmape:.1%}")
...
def main():
    ...
    parser.add_argument(
        "--ref",
        required=True,
        help="Path to reference CSV (e.g., workloads/results/kernel_timings_20260317-075319-odyssey.csv)",
    )
    parser.add_argument(
        "--sim",
        default=None,
