...
Usage:
    python3 compare_regression.py --ref kernel_timings.csv --sim sim_results.csv [--output details.csv]
"""
...
TIMING_COLUMN_SCALES: List[Tuple[str, float]] = [
    ("avg_ms", 1.0),
    ("sim_time_ms", 1.0),
    ("sim_time_sec", 1000.0),
    ("time_ms", 1.0),
...
def _make_skip_record(
    ...
    """Create a deterministic skip-record payload for diagnostics."""
    return {
        "source": source_name,
        "kernel_name": (row.get("kernel_name") or "").strip() or "<missing>",
        "problem_size": (row.get("problem_size") or "").strip() or "<missing>",
        "raw_value": raw_value,
        "reason": reason,
    }


def emit_skipped_row_diagnostics(
    skipped_rows: List[Dict[str, str]],
    sample_limit: int = SKIP_SAMPLE_LIMIT,
) -> None:
    """Print deterministic diagnostics for skipped timing rows."""
    if not skipped_rows:
        return

    print(
        f"[compare_regression] Skipped rows: {len(skipped_rows)}",
        file=sys.stderr,
    )
    ...
    for idx, row in enumerate(sample_rows, start=1):
        raw_value = row["raw_value"] if row["raw_value"] else "<blank>"
        print(
            "[compare_regression] sample[{}] source={} "
            "kernel_name={} problem_size={} raw_value={} reason={}".format(
                idx,
                row["source"],
                row["kernel_name"],
                row["problem_size"],
    ...
    remaining = len(skipped_rows) - len(sample_rows)
    if remaining > 0:
        print(
            f"[compare_regression] ... {remaining} additional skipped row(s) omitted",
            file=sys.stderr,
        )
...
def read_csv_data(
    ...
    """Read CSV timing data and return parsed points and skipped-row diagnostics.

    Supports timing columns: avg_ms, sim_time_ms, sim_time_sec, time_ms, time_sec.
    Invalid timing rows are skipped (blank/non-numeric values).
    """
    ...
    with open(filepath, "r") as f:
        ...
        timing_column = None
        timing_scale = 1.0
        for col, scale in TIMING_COLUMN_SCALES:
        ...
        for row in reader:
            kernel = (row.get("kernel_name") or "").strip()
            size = (row.get("problem_size") or "").strip()
            if not kernel or not size:
                skipped_rows.append(
                    _make_skip_record(
                        source,
                        row,
                        "",
                        "missing kernel_name/problem_size",
                    )
                )
                continue
            ...
            if timing_column:
                raw_value = (row.get(timing_column) or "").strip()
                if not raw_value:
                    skipped_rows.append(
                        _make_skip_record(
                            source,
                            row,
                            raw_value,
                            f"blank {timing_column}",
                        )
                    )
                    continue

                try:
                    ms = float(raw_value) * timing_scale
                except ValueError:
                    skipped_rows.append(
                        _make_skip_record(
                            source,
                ...
                raw_value = ""
                try:
                    raw_candidate = cols[3] if len(cols) > 3 else cols[2]
                    raw_value = (
                        raw_candidate.strip()
                        if isinstance(raw_candidate, str)
                        else str(raw_candidate)
                    )
                    ms = float(raw_candidate)
                    ...
                    skipped_rows.append(
                        _make_skip_record(
                            ...
                            row,
                            raw_value,
                            "unsupported timing column and fallback parse failed",
                        )
                    )
                    continue
...
def main():
    ...
    parser.add_argument(
        "--ref",
        required=True,
        help="Path to hardware reference CSV (e.g., kernel_timings_20260317-075319-odyssey.csv)",
    )
    parser.add_argument(
        "--sim",
        required=True,
    ...
    ref_data, ref_skipped = read_csv_data(args.ref, source_name="ref")
    sim_data, sim_skipped = read_csv_data(args.sim, source_name="sim")
    emit_skipped_row_diagnostics(ref_skipped + sim_skipped)
