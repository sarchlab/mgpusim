...
int main(int argc, char** argv) {
    ...
    srand(42);
    for (int i = 0; i < N; ++i) {
        h_pos[i].x = ((float)rand() / (float)RAND_MAX) * BOX_SIZE;
        h_pos[i].y = ((float)rand() / (float)RAND_MAX) * BOX_SIZE;
        h_pos[i].z = ((float)rand() / (float)RAND_MAX) * BOX_SIZE;
        h_pos[i].w = 0.0f;
    }

    // Device allocations
    float4 *d_pos, *d_force;
    HIP_CHECK(hipMalloc(&d_pos, bytesPos));
    HIP_CHECK(hipMalloc(&d_force, bytesForce));

    HIP_CHECK(hipMemcpy(d_pos, h_pos, bytesPos, hipMemcpyHostToDevice));
    ...
    BenchResult r = runBenchmark("md", size_str, iters, [&]() {
        hipLaunchKernelGGL(md_kernel, dim3(grid), dim3(block_size),
                           0, 0, d_pos, d_force, N, cutoff);
    });
    ...
    if (N <= 1024) {
        HIP_CHECK(hipMemcpy(h_force, d_force, bytesForce,
