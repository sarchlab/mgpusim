// sort.cu -- SHOC Sort benchmark (CUDA)
...
static void scan_recursive_uint(unsigned int* d_output,
    ...
    if (num_blocks > 1) {
        ...
        CUDA_CHECK(cudaMalloc(&d_scanned, num_blocks * sizeof(unsigned int)));
        scan_recursive_uint(d_scanned, d_block_sums, num_blocks, block_size);

        add_block_sums_uint_kernel<<<num_blocks, block_size>>>(
            d_output, d_scanned, N);
...
int main(int argc, char** argv) {
    ...
    CUDA_CHECK(cudaMalloc(&d_predicates, bytes));
    CUDA_CHECK(cudaMalloc(&d_scan_result, bytes));
    ...
    BenchResult r = runBenchmark("sort", size_str, iters, [&]() {
        CUDA_CHECK(cudaMemcpy(d_keys_in, h_keys, bytes,
                              cudaMemcpyHostToDevice));

        for (int bit = 0; bit < 32; ++bit) {
            compute_predicates_kernel<<<grid_padded, block_size>>>(
            ...
            CUDA_CHECK(cudaMemcpy(&last_pred, d_predicates + N - 1,
                                  sizeof(unsigned int),
                                  cudaMemcpyDeviceToHost));
            unsigned int total_zeros = last_scan + last_pred;

            scatter_kernel<<<grid, block_size>>>(
    ...
    CUDA_CHECK(cudaMemcpy(h_result, d_keys_in, (size_t)N * sizeof(unsigned int),
                          cudaMemcpyDeviceToHost));
