// md5hash.cu -- SHOC MD5Hash benchmark (CUDA)
...
int main(int argc, char** argv) {
    ...
    CUDA_CHECK(cudaMalloc(&d_output, bytesOut));
    ...
    BenchResult r = runBenchmark("md5hash", size_str, iters, [&]() {
        md5hash_kernel<<<grid, block_size>>>(d_output, N);
    });
    ...
    CUDA_CHECK(cudaMemcpy(h_output, d_output, bytesOut,
