// DeviceMemory.cu -- SHOC DeviceMemory benchmark (CUDA)
...
int main(int argc, char** argv) {
    ...
    BenchResult r_read = runBenchmark("DeviceMemory_Read", size_str, iters, [&]() {
        read_kernel<<<num_blocks, block_size>>>(d_input, d_output, (int)N);
    });
    printCSVRow(r_read);
    ...
    BenchResult r_write = runBenchmark("DeviceMemory_Write", size_str, iters, [&]() {
        write_kernel<<<num_blocks, block_size>>>(d_output, (int)N);
    });
    printCSVRow(r_write);
