// scan.cpp -- SHOC Scan benchmark (HIP)
...
static void scan_recursive(float* d_output, float* d_input, int N,
    ...
    if (num_blocks > 1) {
        HIP_CHECK(hipMalloc(&d_block_sums, num_blocks * sizeof(float)));
    }
    ...
    if (num_blocks > 1) {
        ...
        HIP_CHECK(hipMalloc(&d_scanned_block_sums,
                             num_blocks * sizeof(float)));
        scan_recursive(d_scanned_block_sums, d_block_sums, num_blocks,
                       block_size);

        // Add scanned block sums back
        hipLaunchKernelGGL(add_block_sums_kernel, dim3(num_blocks),
                           dim3(block_size), 0, 0,
                           d_output, d_scanned_block_sums, N);

        HIP_CHECK(hipFree(d_scanned_block_sums));
...
int main(int argc, char** argv) {
    ...
    BenchResult r = runBenchmark("scan", size_str, iters, [&]() {
        // Need to re-copy input each iteration since scan is in-place-ish
    ...
    // Verify
    // Run one more time for verification
    HIP_CHECK(hipMemcpy(d_output, d_input, bytes, hipMemcpyDeviceToDevice));
    scan_recursive(d_output, d_input, N_padded, block_size);
    HIP_CHECK(hipDeviceSynchronize());
    HIP_CHECK(hipMemcpy(h_output, d_output, (size_t)N * sizeof(float),
