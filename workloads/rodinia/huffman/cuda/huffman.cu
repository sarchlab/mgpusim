// huffman.cu -- CUDA benchmark for Rodinia Huffman Encoding
...
// codebook_codes[symbol] = the Huffman code (packed into 32-bit int)
// codebook_lengths[symbol] = number of valid bits in the code
...
int main(int argc, char **argv) {
    ...
    CUDA_CHECK(cudaMalloc(&d_data, data_size * sizeof(unsigned char)));
    CUDA_CHECK(cudaMalloc(&d_hist, 256 * sizeof(unsigned int)));
    CUDA_CHECK(cudaMalloc(&d_codebook_codes, 256 * sizeof(unsigned int)));
    CUDA_CHECK(cudaMalloc(&d_codebook_lengths, 256 * sizeof(unsigned int)));
    CUDA_CHECK(cudaMalloc(&d_output_codes, data_size * sizeof(unsigned int)));
    CUDA_CHECK(
        cudaMalloc(&d_output_lengths, data_size * sizeof(unsigned int)));
    ...
    BenchResult r = runBenchmark("huffman", problemSize, iterations, [&]() {
        ...
        // Download histogram
        CUDA_CHECK(cudaMemcpy(h_hist, d_hist, 256 * sizeof(unsigned int),
        ...
        // Upload codebook
        CUDA_CHECK(cudaMemcpy(d_codebook_codes, h_codes,
                              256 * sizeof(unsigned int),
                              cudaMemcpyHostToDevice));
        CUDA_CHECK(cudaMemcpy(d_codebook_lengths, h_lengths,
                              256 * sizeof(unsigned int),
                              cudaMemcpyHostToDevice));
    ...
    CUDA_CHECK(cudaFree(d_codebook_codes));
    CUDA_CHECK(cudaFree(d_codebook_lengths));
    CUDA_CHECK(cudaFree(d_output_codes));
    CUDA_CHECK(cudaFree(d_output_lengths));
    free(h_data);
