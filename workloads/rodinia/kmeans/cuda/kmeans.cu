// kmeans.cu -- CUDA benchmark for Rodinia K-Means Clustering
...
int main(int argc, char **argv) {
    ...
    int *d_assignments;
    CUDA_CHECK(
        cudaMalloc(&d_points, num_points * num_features * sizeof(float)));
    CUDA_CHECK(
        cudaMalloc(&d_centers, num_clusters * num_features * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&d_assignments, num_points * sizeof(int)));
    ...
    BenchResult r = runBenchmark("kmeans", problemSize, iterations, [&]() {
        ...
        for (int iter = 0; iter < max_iterations; iter++) {
            // Upload centers
            CUDA_CHECK(cudaMemcpy(d_centers, h_centers,
            ...
            CUDA_CHECK(cudaMemcpy(h_assignments, d_assignments,
