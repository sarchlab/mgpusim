// lavaMD.cpp -- HIP benchmark for Rodinia LavaMD
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < total_boxes; i++) {
        ...
        int bx_idx = i % boxes_per_dim;
        float cx = bx_idx * cutoff_radius;
    ...
    HIP_CHECK(hipMalloc(&d_rv, total_particles * sizeof(FOUR_VECTOR)));
    HIP_CHECK(hipMalloc(&d_fv, total_particles * sizeof(FOUR_VECTOR)));
    HIP_CHECK(hipMalloc(&d_qv, total_particles * sizeof(float)));
    HIP_CHECK(hipMalloc(&d_box_neighbors,
                        total_boxes * max_neighbors * sizeof(int)));
    HIP_CHECK(hipMalloc(&d_box_num_neighbors,
                        total_boxes * sizeof(int)));

    // Copy data to device
    HIP_CHECK(hipMemcpy(d_rv, h_rv,
                        total_particles * sizeof(FOUR_VECTOR),
                        hipMemcpyHostToDevice));
    HIP_CHECK(hipMemcpy(d_qv, h_qv, total_particles * sizeof(float),
                        hipMemcpyHostToDevice));
    HIP_CHECK(hipMemcpy(d_box_neighbors, h_box_neighbors,
                        total_boxes * max_neighbors * sizeof(int),
                        hipMemcpyHostToDevice));
    HIP_CHECK(hipMemcpy(d_box_num_neighbors, h_box_num_neighbors,
                        total_boxes * sizeof(int),
                        hipMemcpyHostToDevice));
    ...
    // Cleanup
    HIP_CHECK(hipFree(d_rv));
    HIP_CHECK(hipFree(d_fv));
    HIP_CHECK(hipFree(d_qv));
    HIP_CHECK(hipFree(d_box_neighbors));
    HIP_CHECK(hipFree(d_box_num_neighbors));
    free(h_rv);
    free(h_qv);
    free(h_fv);
