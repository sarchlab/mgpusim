// backprop.cpp -- HIP benchmark for Rodinia Back Propagation
...
int main(int argc, char **argv) {
    ...
    HIP_CHECK(hipMalloc(&d_input, num_input * sizeof(DATA_TYPE)));
    HIP_CHECK(
        hipMalloc(&d_weights, input_weights_size * sizeof(DATA_TYPE)));
    HIP_CHECK(
        hipMalloc(&d_prev_weights, input_weights_size * sizeof(DATA_TYPE)));
    HIP_CHECK(hipMalloc(&d_hidden, num_hidden * sizeof(DATA_TYPE)));
    HIP_CHECK(hipMalloc(&d_delta, num_hidden * sizeof(DATA_TYPE)));

    HIP_CHECK(hipMemcpy(d_input, h_input, num_input * sizeof(DATA_TYPE),
                        hipMemcpyHostToDevice));
    HIP_CHECK(hipMemcpy(d_weights, h_weights,
                        input_weights_size * sizeof(DATA_TYPE),
                        hipMemcpyHostToDevice));
    HIP_CHECK(hipMemcpy(d_prev_weights, h_prev_weights,
                        input_weights_size * sizeof(DATA_TYPE),
                        hipMemcpyHostToDevice));
    HIP_CHECK(hipMemcpy(d_delta, h_delta, num_hidden * sizeof(DATA_TYPE),
                        hipMemcpyHostToDevice));
    ...
    HIP_CHECK(hipFree(d_prev_weights));
