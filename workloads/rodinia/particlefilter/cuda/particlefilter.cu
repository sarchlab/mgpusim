// particlefilter.cu -- CUDA benchmark for Rodinia Particle Filter
...
int main(int argc, char **argv) {
    ...
    CUDA_CHECK(cudaMalloc(&d_particles,
                          num_particles * state_dims * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&d_particles_tmp,
                          num_particles * state_dims * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&d_weights, num_particles * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&d_cdf, num_particles * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&d_obs, state_dims * sizeof(float)));
    ...
    BenchResult r = runBenchmark(
        "particlefilter", problemSize, iterations, [&]() {
            ...
            for (int t = 0; t < num_timesteps; t++) {
                ...
                CUDA_CHECK(cudaMemcpy(d_obs, &h_observations[t * state_dims],
                ...
                // 2. Normalize weights
                float zero = 0.0f;
                CUDA_CHECK(cudaMemcpy(d_weight_sum, &zero, sizeof(float),
                                      cudaMemcpyHostToDevice));
    ...
    CUDA_CHECK(cudaFree(d_particles_tmp));
    CUDA_CHECK(cudaFree(d_weights));
    CUDA_CHECK(cudaFree(d_cdf));
    CUDA_CHECK(cudaFree(d_obs));
    CUDA_CHECK(cudaFree(d_weight_sum));
    free(h_particles);
    free(h_observations);
