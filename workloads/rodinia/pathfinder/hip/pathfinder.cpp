// pathfinder.cpp -- HIP benchmark for Rodinia Pathfinder
...
int main(int argc, char **argv) {
    ...
    HIP_CHECK(hipMalloc(&d_wall, total_cells * sizeof(DATA_TYPE)));
    HIP_CHECK(hipMalloc(&d_src, width * sizeof(DATA_TYPE)));
    HIP_CHECK(hipMalloc(&d_dst, width * sizeof(DATA_TYPE)));

    HIP_CHECK(hipMemcpy(d_wall, h_wall, total_cells * sizeof(DATA_TYPE),
    ...
    BenchResult r = runBenchmark(
        "pathfinder", problemSize, iterations, [&]() {
            // Reset first row
            HIP_CHECK(hipMemcpy(d_src, h_result, width * sizeof(DATA_TYPE),
                                hipMemcpyHostToDevice));
