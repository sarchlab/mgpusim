// gaussian.cpp -- HIP benchmark for Rodinia Gaussian Elimination
...
int main(int argc, char **argv) {
    ...
    HIP_CHECK(hipMalloc(&d_a, size * size * sizeof(DATA_TYPE)));
    HIP_CHECK(hipMalloc(&d_b, size * sizeof(DATA_TYPE)));
    HIP_CHECK(hipMalloc(&d_m, size * size * sizeof(DATA_TYPE)));
    ...
    BenchResult r = runBenchmark("gaussian", problemSize, iterations, [&]() {
        ...
        HIP_CHECK(hipMemcpy(d_a, h_a, size * size * sizeof(DATA_TYPE),
                            hipMemcpyHostToDevice));
        HIP_CHECK(hipMemcpy(d_b, h_b, size * sizeof(DATA_TYPE),
                            hipMemcpyHostToDevice));
        HIP_CHECK(hipMemcpy(d_m, h_m, size * size * sizeof(DATA_TYPE),
                            hipMemcpyHostToDevice));
