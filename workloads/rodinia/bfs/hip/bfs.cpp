// bfs.cpp -- HIP benchmark for Rodinia Breadth-First Search
...
int main(int argc, char **argv) {
    ...
    HIP_CHECK(hipMalloc(&d_cost, num_nodes * sizeof(int)));
    HIP_CHECK(hipMalloc(&d_frontier, num_nodes * sizeof(int)));
    HIP_CHECK(hipMalloc(&d_visited, num_nodes * sizeof(int)));
    HIP_CHECK(hipMalloc(&d_updated, sizeof(int)));
    ...
    BenchResult r = runBenchmark("bfs", problemSize, iterations, [&]() {
        ...
        HIP_CHECK(hipMemcpy(d_cost, h_cost, num_nodes * sizeof(int),
                            hipMemcpyHostToDevice));
        HIP_CHECK(hipMemcpy(d_frontier, h_frontier,
                            num_nodes * sizeof(int),
                            hipMemcpyHostToDevice));
        HIP_CHECK(hipMemcpy(d_visited, h_visited,
                            num_nodes * sizeof(int),
                            hipMemcpyHostToDevice));
        ...
        while (h_updated) {
            h_updated = 0;
            HIP_CHECK(hipMemcpy(d_updated, &h_updated, sizeof(int),
                                hipMemcpyHostToDevice));
            ...
            HIP_CHECK(hipMemcpy(&h_updated, d_updated, sizeof(int),
