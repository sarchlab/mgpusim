// bh.cpp -- HIP benchmark for Lonestar Barnes-Hut n-body simulation
...
int main(int argc, char **argv) {
    ...
    TreeNode *d_tree;
    HIP_CHECK(hipMalloc(&d_px, num_bodies * sizeof(float)));
    HIP_CHECK(hipMalloc(&d_py, num_bodies * sizeof(float)));
    HIP_CHECK(hipMalloc(&d_vx, num_bodies * sizeof(float)));
    HIP_CHECK(hipMalloc(&d_vy, num_bodies * sizeof(float)));
    HIP_CHECK(hipMalloc(&d_mass, num_bodies * sizeof(float)));
    HIP_CHECK(hipMalloc(&d_fx, num_bodies * sizeof(float)));
    HIP_CHECK(hipMalloc(&d_fy, num_bodies * sizeof(float)));
    HIP_CHECK(hipMalloc(&d_tree, max_tree_nodes * sizeof(TreeNode)));
    ...
    BenchResult r = runBenchmark("bh", problemSize, iterations, [&]() {
        ...
        HIP_CHECK(hipMemcpy(d_px, h_px, num_bodies * sizeof(float),
                             hipMemcpyHostToDevice));
        HIP_CHECK(hipMemcpy(d_py, h_py, num_bodies * sizeof(float),
                             hipMemcpyHostToDevice));
        HIP_CHECK(hipMemcpy(d_vx, h_vx, num_bodies * sizeof(float),
                             hipMemcpyHostToDevice));
        HIP_CHECK(hipMemcpy(d_vy, h_vy, num_bodies * sizeof(float),
                             hipMemcpyHostToDevice));
        HIP_CHECK(hipMemcpy(d_mass, h_mass, num_bodies * sizeof(float),
                             hipMemcpyHostToDevice));

        for (int step = 0; step < num_timesteps; step++) {
            // Copy positions back to host for tree building
            if (step > 0) {
                HIP_CHECK(hipMemcpy(h_px, d_px, num_bodies * sizeof(float),
                                    hipMemcpyDeviceToHost));
                HIP_CHECK(hipMemcpy(h_py, d_py, num_bodies * sizeof(float),
                                    hipMemcpyDeviceToHost));
            }
            ...
            // Copy tree to device
            HIP_CHECK(hipMemcpy(d_tree, h_tree,
    ...
    HIP_CHECK(hipFree(d_mass));
    HIP_CHECK(hipFree(d_fx));
    HIP_CHECK(hipFree(d_fy));
