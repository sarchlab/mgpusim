// mst.cpp -- HIP benchmark for Lonestar Minimum Spanning Tree
...
// component.  Store the result in min_edge[comp] using atomics
// (pack weight+edge_id into a long long for atomic compare).
...
int main(int argc, char **argv) {
    ...
    HIP_CHECK(hipMalloc(&d_edge_weights, num_edges * sizeof(int)));
    HIP_CHECK(hipMalloc(&d_parent, num_nodes * sizeof(int)));
    HIP_CHECK(hipMalloc(&d_min_edge, num_nodes * sizeof(long long)));
    HIP_CHECK(hipMalloc(&d_changed, sizeof(int)));
    ...
    HIP_CHECK(hipMemcpy(d_col_indices, h_col_indices,
                        ...
                        hipMemcpyHostToDevice));
    HIP_CHECK(hipMemcpy(d_edge_weights, h_edge_weights,
                        num_edges * sizeof(int),
                        hipMemcpyHostToDevice));
    ...
    BenchResult r = runBenchmark("mst", problemSize, iterations, [&]() {
        ...
        HIP_CHECK(hipMemcpy(d_parent, h_parent, num_nodes * sizeof(int),
                            hipMemcpyHostToDevice));

        int h_changed = 1;
        while (h_changed) {
            // Reset min_edge to LLONG_MAX
            HIP_CHECK(hipMemset(d_min_edge, 0x7F,
            ...
            h_changed = 0;
            HIP_CHECK(hipMemcpy(d_changed, &h_changed, sizeof(int),
                                hipMemcpyHostToDevice));
            ...
            HIP_CHECK(hipMemcpy(&h_changed, d_changed, sizeof(int),
    ...
    HIP_CHECK(hipFree(d_edge_weights));
    HIP_CHECK(hipFree(d_parent));
    HIP_CHECK(hipFree(d_min_edge));
