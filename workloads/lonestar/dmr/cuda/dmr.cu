// dmr.cu -- CUDA benchmark for Lonestar Delaunay Mesh Refinement
...
int main(int argc, char **argv) {
    ...
    CUDA_CHECK(cudaMalloc(&d_px, max_points * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&d_py, max_points * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&d_triangles, max_triangles * sizeof(Triangle)));
    CUDA_CHECK(cudaMalloc(&d_is_bad, max_triangles * sizeof(int)));
    CUDA_CHECK(cudaMalloc(&d_tri_locks, max_triangles * sizeof(int)));
    CUDA_CHECK(cudaMalloc(&d_num_bad, sizeof(int)));
    CUDA_CHECK(cudaMalloc(&d_point_count, sizeof(int)));
    ...
    BenchResult r = runBenchmark("dmr", problemSize, iterations, [&]() {
        ...
        // Copy to device
        CUDA_CHECK(cudaMemcpy(d_px, h_px, h_point_count * sizeof(float),
                              cudaMemcpyHostToDevice));
        CUDA_CHECK(cudaMemcpy(d_py, h_py, h_point_count * sizeof(float),
                              cudaMemcpyHostToDevice));
        CUDA_CHECK(cudaMemcpy(d_triangles, h_triangles,
                              h_tri_count * sizeof(Triangle),
                              cudaMemcpyHostToDevice));
        CUDA_CHECK(cudaMemcpy(d_point_count, &h_point_count, sizeof(int),
                              cudaMemcpyHostToDevice));
        CUDA_CHECK(cudaMemcpy(d_tri_count, &h_tri_count, sizeof(int),
                              cudaMemcpyHostToDevice));

        // Iterative refinement
        for (int pass = 0; pass < MAX_PASSES; pass++) {
            int h_num_bad = 0;
            CUDA_CHECK(cudaMemcpy(d_num_bad, &h_num_bad, sizeof(int),
                                  cudaMemcpyHostToDevice));

            // Read current triangle count
            CUDA_CHECK(cudaMemcpy(&h_tri_count, d_tri_count, sizeof(int),
            ...
            CUDA_CHECK(cudaMemcpy(&h_num_bad, d_num_bad, sizeof(int),
                                  cudaMemcpyDeviceToHost));
            if (h_num_bad == 0) break;

            // Reset locks
            CUDA_CHECK(cudaMemset(d_tri_locks, 0,
    ...
    CUDA_CHECK(cudaFree(d_is_bad));
    CUDA_CHECK(cudaFree(d_tri_locks));
    CUDA_CHECK(cudaFree(d_num_bad));
