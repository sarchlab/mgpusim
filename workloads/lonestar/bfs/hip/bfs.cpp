// bfs.cpp -- HIP benchmark for Lonestar Breadth-First Search
...
int main(int argc, char **argv) {
    ...
    HIP_CHECK(hipMalloc(&d_row_offsets, (num_nodes + 1) * sizeof(int)));
    HIP_CHECK(hipMalloc(&d_col_indices, num_edges * sizeof(int)));
    HIP_CHECK(hipMalloc(&d_dist, num_nodes * sizeof(int)));
    HIP_CHECK(hipMalloc(&d_visited, num_nodes * sizeof(int)));
    // Worklists -- each can hold up to num_nodes entries
    HIP_CHECK(hipMalloc(&d_worklist_a, num_nodes * sizeof(int)));
    HIP_CHECK(hipMalloc(&d_worklist_b, num_nodes * sizeof(int)));
    HIP_CHECK(hipMalloc(&d_wl_size, sizeof(int)));

    // Copy graph structure to device (constant across iterations)
    HIP_CHECK(hipMemcpy(d_row_offsets, h_row_offsets,
                        (num_nodes + 1) * sizeof(int),
                        hipMemcpyHostToDevice));
    HIP_CHECK(hipMemcpy(d_col_indices, h_col_indices,
    ...
    BenchResult r = runBenchmark("bfs", problemSize, iterations, [&]() {
        ...
        HIP_CHECK(hipMemset(d_dist, -1, num_nodes * sizeof(int)));
        HIP_CHECK(hipMemset(d_visited, 0, num_nodes * sizeof(int)));

        // Source node = 0
        int zero = 0;
        int one = 1;
        HIP_CHECK(hipMemcpy(&d_dist[0], &zero, sizeof(int),
                             hipMemcpyHostToDevice));
        HIP_CHECK(hipMemcpy(&d_visited[0], &one, sizeof(int),
                             hipMemcpyHostToDevice));

        // Initialize input worklist with source node
        HIP_CHECK(hipMemcpy(d_worklist_a, &zero, sizeof(int),
                             hipMemcpyHostToDevice));
        int h_in_size = 1;
        ...
        while (h_in_size > 0) {
            ...
            HIP_CHECK(hipMemset(d_wl_size, 0, sizeof(int)));
            ...
            HIP_CHECK(hipMemcpy(&h_in_size, d_wl_size, sizeof(int),
    ...
    HIP_CHECK(hipFree(d_visited));
    HIP_CHECK(hipFree(d_worklist_a));
    HIP_CHECK(hipFree(d_worklist_b));
    HIP_CHECK(hipFree(d_wl_size));
    free(h_row_offsets);
    free(h_col_indices);
