// global_bandwidth.cpp -- HIP benchmark for global memory bandwidth
...
__global__ void strided_copy_kernel(const float *d_in, float *d_out,
                                    int N, int stride) {
    int gid = (hipBlockIdx_x * hipBlockDim_x + hipThreadIdx_x) * stride;
    if (gid < N) {
        d_out[gid] = d_in[gid];
    }
}
...
int main(int argc, char **argv) {
    ...
    if (access_type == 1) {
        ...
        BenchResult r = runBenchmark("global_bw_read", problemSize, iterations, [&]() {
            hipLaunchKernelGGL(read_kernel, grid, block, 0, 0,
                               d_in, d_out, N);
        });
        printCSVRow(r);
        ...
        BenchResult r = runBenchmark("global_bw_write", problemSize, iterations, [&]() {
            hipLaunchKernelGGL(write_kernel, grid, block, 0, 0,
                               d_out, N);
        });
        printCSVRow(r);
        ...
        if (stride > 1) {
            ...
            BenchResult r = runBenchmark("global_bw_strided_copy", problemSize, iterations, [&]() {
                hipLaunchKernelGGL(strided_copy_kernel, grid, block, 0, 0,
                                   d_in, d_out, effective_N, stride);
            });
            printCSVRow(r);
            ...
            BenchResult r = runBenchmark("global_bw_copy", problemSize, iterations, [&]() {
                hipLaunchKernelGGL(copy_kernel, grid, block, 0, 0,
                                   d_in, d_out, N);
            });
            printCSVRow(r);
