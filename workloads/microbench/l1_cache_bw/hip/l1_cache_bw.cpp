// l1_cache_bw.cpp -- HIP benchmark for L1 cache bandwidth
...
// remains a non-scaling L1-resident footprint for this HIP path. The Go
...
int main(int argc, char **argv) {
    ...
    // Device allocations.
    float *d_data, *d_out;
    HIP_CHECK(hipMalloc(&d_data, working_set_size));
    HIP_CHECK(hipMalloc(&d_out, total_threads * sizeof(float)));
    HIP_CHECK(hipMemcpy(d_data, h_data, working_set_size,
                        hipMemcpyHostToDevice));
    HIP_CHECK(hipMemset(d_out, 0, total_threads * sizeof(float)));
    ...
    BenchResult r = runBenchmark("l1_cache_bw", problemSize, iterations, [&]() {
        hipLaunchKernelGGL(l1_cache_bw_kernel, grid, block, 0, 0,
                           d_data, d_out, working_set_elems, num_iters,
