// branch_divergence.cpp -- HIP benchmark for branch divergence penalty measurement
...
// divergence_threshold: number of lanes (0-64) that take the "if" branch
...
__global__ void branch_divergence_kernel(float *d_data, int num_elements,
    ...
    int warp_size = 64; // AMD wavefront size
...
int main(int argc, char **argv) {
    ...
    // Convert divergence percentage to threshold
    // For HIP/AMD, wavefront size is 64
    int warp_size = 64;
    ...
    BenchResult r = runBenchmark(kernel_name, problemSize, iterations, [&]() {
        ...
        HIP_CHECK(hipMemcpy(d_data, h_data, num_elements * sizeof(float),
                             hipMemcpyHostToDevice));
        hipLaunchKernelGGL(branch_divergence_kernel, grid, block, 0, 0,
                           d_data, num_elements, divergence_threshold,
                           work_per_branch);
    });
    printCSVRow(r);
