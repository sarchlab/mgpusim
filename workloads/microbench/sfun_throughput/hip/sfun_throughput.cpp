// sfun_throughput.cpp -- HIP benchmark for special function unit throughput
...
int main(int argc, char **argv) {
    ...
    float *d_out;
    HIP_CHECK(hipMalloc(&d_out, num_threads_total * sizeof(float)));
    HIP_CHECK(hipMemset(d_out, 0, num_threads_total * sizeof(float)));
    ...
    switch (func_type) {
        case 1:
            r = runBenchmark(kernel_name, problemSize, iterations, [&]() {
                hipLaunchKernelGGL(sfun_sin_kernel, grid, block, 0, 0,
                                   num_ops, d_out);
            });
            break;
        case 2:
            r = runBenchmark(kernel_name, problemSize, iterations, [&]() {
                hipLaunchKernelGGL(sfun_cos_kernel, grid, block, 0, 0,
                                   num_ops, d_out);
            });
            break;
        case 3:
            r = runBenchmark(kernel_name, problemSize, iterations, [&]() {
                hipLaunchKernelGGL(sfun_exp_kernel, grid, block, 0, 0,
                                   num_ops, d_out);
            });
            break;
        case 4:
            r = runBenchmark(kernel_name, problemSize, iterations, [&]() {
                hipLaunchKernelGGL(sfun_sqrt_kernel, grid, block, 0, 0,
                                   num_ops, d_out);
            });
            break;
        case 5:
            r = runBenchmark(kernel_name, problemSize, iterations, [&]() {
                hipLaunchKernelGGL(sfun_rsqrt_kernel, grid, block, 0, 0,
                                   num_ops, d_out);
            });
            break;
    }
