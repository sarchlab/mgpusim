// mem_latency.cpp -- HIP benchmark for memory latency via pointer chasing
...
int main(int argc, char **argv) {
    ...
    int *d_array;
    long long *d_cycles;
    HIP_CHECK(hipMalloc(&d_array, array_size * sizeof(int)));
    HIP_CHECK(hipMalloc(&d_cycles, num_threads * sizeof(long long)));
    HIP_CHECK(hipMemcpy(d_array, h_array, array_size * sizeof(int),
    ...
    BenchResult r = runBenchmark("mem_latency_chase", problemSize, iterations, [&]() {
        hipLaunchKernelGGL(pointer_chase_kernel, grid, block, 0, 0,
                           d_array, array_size, num_hops, d_cycles);
    });
    printCSVRow(r);

    // Retrieve device-side cycle counts for reporting
    long long *h_cycles = (long long *)malloc(num_threads * sizeof(long long));
    HIP_CHECK(hipMemcpy(h_cycles, d_cycles, num_threads * sizeof(long long),
