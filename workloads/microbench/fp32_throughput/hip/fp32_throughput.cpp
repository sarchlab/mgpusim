// fp32_throughput.cpp -- HIP benchmark for single-precision floating-point throughput
...
int main(int argc, char **argv) {
    ...
    if (op_type == 1) {
        ...
        BenchResult r = runBenchmark("fp32_add", problemSize, iterations, [&]() {
            hipLaunchKernelGGL(fp32_add_kernel, grid, block, 0, 0,
                               num_ops, d_out);
        });
        printCSVRow(r);
        ...
        BenchResult r = runBenchmark("fp32_mul", problemSize, iterations, [&]() {
            hipLaunchKernelGGL(fp32_mul_kernel, grid, block, 0, 0,
                               num_ops, d_out);
        });
        printCSVRow(r);
        ...
        BenchResult r = runBenchmark("fp32_fma", problemSize, iterations, [&]() {
            hipLaunchKernelGGL(fp32_fma_kernel, grid, block, 0, 0,
                               num_ops, d_out);
        });
        printCSVRow(r);
