// fp64_throughput.cu -- CUDA benchmark for double-precision floating-point throughput
...
int main(int argc, char **argv) {
    ...
    double *d_out;
    CUDA_CHECK(cudaMalloc(&d_out, num_threads_total * sizeof(double)));
    CUDA_CHECK(cudaMemset(d_out, 0, num_threads_total * sizeof(double)));
    ...
    if (op_type == 1) {
        ...
        BenchResult r = runBenchmark("fp64_add", problemSize, iterations, [&]() {
            fp64_add_kernel<<<grid, block>>>(num_ops, d_out);
        });
        printCSVRow(r);
        ...
        BenchResult r = runBenchmark("fp64_mul", problemSize, iterations, [&]() {
            fp64_mul_kernel<<<grid, block>>>(num_ops, d_out);
        });
        printCSVRow(r);
        ...
        BenchResult r = runBenchmark("fp64_fma", problemSize, iterations, [&]() {
            fp64_fma_kernel<<<grid, block>>>(num_ops, d_out);
        });
        printCSVRow(r);
