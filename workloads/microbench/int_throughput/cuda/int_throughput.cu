// int_throughput.cu -- CUDA benchmark for integer arithmetic throughput
...
int main(int argc, char **argv) {
    ...
    int *d_out;
    CUDA_CHECK(cudaMalloc(&d_out, num_threads_total * sizeof(int)));
    CUDA_CHECK(cudaMemset(d_out, 0, num_threads_total * sizeof(int)));
    ...
    if (op_type == 1) {
        BenchResult r = runBenchmark("int_add", problemSize, iterations, [&]() {
            int_add_kernel<<<grid, block>>>(num_ops, d_out);
        });
        printCSVRow(r);
        ...
        BenchResult r = runBenchmark("int_mul", problemSize, iterations, [&]() {
            int_mul_kernel<<<grid, block>>>(num_ops, d_out);
        });
        printCSVRow(r);
        ...
        BenchResult r = runBenchmark("int_mad", problemSize, iterations, [&]() {
            int_mad_kernel<<<grid, block>>>(num_ops, d_out);
        });
        printCSVRow(r);
