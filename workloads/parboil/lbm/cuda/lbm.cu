// lbm.cu -- CUDA benchmark for Parboil Lattice-Boltzmann Method (D3Q19)
...
int main(int argc, char **argv) {
    ...
    float *d_src, *d_dst;
    CUDA_CHECK(cudaMalloc(&d_src, lattice_size));
    CUDA_CHECK(cudaMalloc(&d_dst, lattice_size));

    CUDA_CHECK(cudaMemcpy(d_src, h_lattice, lattice_size,
