// mri-gridding.cu -- CUDA benchmark for Parboil MRI Reconstruction via Gridding
...
int main(int argc, char **argv) {
    ...
    // Kaiser-Bessel beta parameter (typical value for MRI gridding)
    float beta = (float)M_PI * sqrtf(
        (float)(kernel_width * kernel_width) /
        (float)(kernel_width * kernel_width) * 4.0f - 1.0f);
    if (beta != beta) beta = 5.0f; // fallback for NaN
    beta = 5.0f * (float)kernel_width / 4.0f; // simplified standard choice
    ...
    CUDA_CHECK(cudaMalloc(&d_sx, samples_bytes));
    CUDA_CHECK(cudaMalloc(&d_sy, samples_bytes));
    CUDA_CHECK(cudaMalloc(&d_sr, samples_bytes));
    CUDA_CHECK(cudaMalloc(&d_si, samples_bytes));
    CUDA_CHECK(cudaMalloc(&d_grid_real, grid_bytes));
    CUDA_CHECK(cudaMalloc(&d_grid_imag, grid_bytes));

    CUDA_CHECK(cudaMemcpy(d_sx, h_sx, samples_bytes, cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_sy, h_sy, samples_bytes, cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_sr, h_sr, samples_bytes, cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_si, h_si, samples_bytes, cudaMemcpyHostToDevice));
    ...
    BenchResult r = runBenchmark(
        "gridding_kernel", problemSize, iterations, [&]() {
            ...
            CUDA_CHECK(cudaMemset(d_grid_real, 0, grid_bytes));
            CUDA_CHECK(cudaMemset(d_grid_imag, 0, grid_bytes));
    ...
    CUDA_CHECK(cudaFree(d_sr));
    CUDA_CHECK(cudaFree(d_si));
    CUDA_CHECK(cudaFree(d_grid_real));
    CUDA_CHECK(cudaFree(d_grid_imag));
    free(h_sx);
    free(h_sy);
    free(h_sr);
    free(h_si);
