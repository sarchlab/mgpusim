// spmv.cu -- CUDA benchmark for Parboil SpMV (Sparse Matrix-Vector Multiplication)
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < num_rows; i++) {
        ...
        // Use simple modular approach to spread columns
        for (int j = 0; j < nnz; j++) {
    ...
    CUDA_CHECK(cudaMalloc(&d_x, num_rows * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&d_y, num_rows * sizeof(float)));

    CUDA_CHECK(cudaMemcpy(d_row_ptr, h_row_ptr, (num_rows + 1) * sizeof(int),
    ...
    CUDA_CHECK(cudaMemcpy(d_values, h_values, total_nnz * sizeof(float),
                          cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_x, h_x, num_rows * sizeof(float),
                          cudaMemcpyHostToDevice));
