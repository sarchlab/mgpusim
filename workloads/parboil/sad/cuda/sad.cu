// sad.cu -- CUDA benchmark for Parboil SAD (Sum of Absolute Differences)
...
__global__ void ComputeSAD(unsigned char *ref_frame, unsigned char *cur_frame,
    ...
    for (int c = tid; c < total_candidates; c += blockDim.x) {
...
__global__ void FindMinSAD(int *sad_values, int *min_sad, int *min_pos,
    ...
    int mb = blockIdx.x * blockDim.x + threadIdx.x;
...
int main(int argc, char **argv) {
    ...
    CUDA_CHECK(cudaMalloc(&d_ref, frame_size));
    CUDA_CHECK(cudaMalloc(&d_cur, frame_size));
    CUDA_CHECK(cudaMalloc(&d_sad_out,
        (long long)num_macroblocks * total_candidates * sizeof(int)));
    CUDA_CHECK(cudaMalloc(&d_min_sad, num_macroblocks * sizeof(int)));
    CUDA_CHECK(cudaMalloc(&d_min_pos, num_macroblocks * sizeof(int)));

    CUDA_CHECK(cudaMemcpy(d_ref, h_ref, frame_size,
                          cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_cur, h_cur, frame_size,
                          cudaMemcpyHostToDevice));
    ...
    CUDA_CHECK(cudaFree(d_sad_out));
    CUDA_CHECK(cudaFree(d_min_sad));
