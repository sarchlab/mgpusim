// cutcp.cu -- CUDA benchmark for Parboil Short-range Electrostatic Potential
...
int main(int argc, char **argv) {
    ...
    CUDA_CHECK(cudaMalloc(&d_atoms, num_atoms * sizeof(Atom)));
    CUDA_CHECK(cudaMalloc(&d_potential, total_points * sizeof(DATA_TYPE)));

    CUDA_CHECK(cudaMemcpy(d_atoms, h_atoms, num_atoms * sizeof(Atom),
                          cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_potential, h_potential,
    ...
    BenchResult r1 = runBenchmark(
        "cutoff_potential", problemSize, iterations, [&]() {
            ...
            cudaMemset(d_potential, 0, total_points * sizeof(DATA_TYPE));
