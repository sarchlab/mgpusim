// tpacf.cu -- CUDA benchmark for Parboil Two-Point Angular Correlation Function
...
int main(int argc, char **argv) {
    ...
    CUDA_CHECK(cudaMalloc(&d_ra, num_points * sizeof(DATA_TYPE)));
    CUDA_CHECK(cudaMalloc(&d_dec, num_points * sizeof(DATA_TYPE)));
    CUDA_CHECK(cudaMalloc(&d_hist, num_bins * sizeof(unsigned long long)));
    ...
    CUDA_CHECK(cudaMemcpy(d_ra, h_data_ra, num_points * sizeof(DATA_TYPE),
                          cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_dec, h_data_dec, num_points * sizeof(DATA_TYPE),
                          cudaMemcpyHostToDevice));

    BenchResult r1 = runBenchmark(
        "tpacf_DD", problemSize, iterations, [&]() {
            CUDA_CHECK(cudaMemset(d_hist, 0, num_bins * sizeof(unsigned long long)));
    ...
    CUDA_CHECK(cudaMemcpy(d_ra, h_rand_ra[0], num_points * sizeof(DATA_TYPE),
                          cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_dec, h_rand_dec[0], num_points * sizeof(DATA_TYPE),
                          cudaMemcpyHostToDevice));
