// mri-q.cpp -- HIP benchmark for Parboil MRI-Q matrix computation
...
int main(int argc, char **argv) {
    ...
    HIP_CHECK(hipMalloc(&d_x, num_voxels * sizeof(float)));
    HIP_CHECK(hipMalloc(&d_y, num_voxels * sizeof(float)));
    HIP_CHECK(hipMalloc(&d_z, num_voxels * sizeof(float)));
    HIP_CHECK(hipMalloc(&d_kx, num_k_samples * sizeof(float)));
    HIP_CHECK(hipMalloc(&d_ky, num_k_samples * sizeof(float)));
    HIP_CHECK(hipMalloc(&d_kz, num_k_samples * sizeof(float)));
    HIP_CHECK(hipMalloc(&d_phi_r, num_k_samples * sizeof(float)));
    HIP_CHECK(hipMalloc(&d_phi_i, num_k_samples * sizeof(float)));
    HIP_CHECK(hipMalloc(&d_Qr, num_voxels * sizeof(float)));
    HIP_CHECK(hipMalloc(&d_Qi, num_voxels * sizeof(float)));

    HIP_CHECK(hipMemcpy(d_x, h_x, num_voxels * sizeof(float),
                        hipMemcpyHostToDevice));
    HIP_CHECK(hipMemcpy(d_y, h_y, num_voxels * sizeof(float),
                        hipMemcpyHostToDevice));
    HIP_CHECK(hipMemcpy(d_z, h_z, num_voxels * sizeof(float),
                        hipMemcpyHostToDevice));
    HIP_CHECK(hipMemcpy(d_kx, h_kx, num_k_samples * sizeof(float),
                        hipMemcpyHostToDevice));
    HIP_CHECK(hipMemcpy(d_ky, h_ky, num_k_samples * sizeof(float),
                        hipMemcpyHostToDevice));
    HIP_CHECK(hipMemcpy(d_kz, h_kz, num_k_samples * sizeof(float),
                        hipMemcpyHostToDevice));
    HIP_CHECK(hipMemcpy(d_phi_r, h_phi_r, num_k_samples * sizeof(float),
                        hipMemcpyHostToDevice));
    HIP_CHECK(hipMemcpy(d_phi_i, h_phi_i, num_k_samples * sizeof(float),
                        hipMemcpyHostToDevice));
    HIP_CHECK(hipMemcpy(d_Qr, h_Qr, num_voxels * sizeof(float),
                        hipMemcpyHostToDevice));
    HIP_CHECK(hipMemcpy(d_Qi, h_Qi, num_voxels * sizeof(float),
                        hipMemcpyHostToDevice));
    ...
    BenchResult r1 = runBenchmark(
        "ComputeQ", problemSize, iterations, [&]() {
            ...
            HIP_CHECK(hipMemset(d_Qr, 0, num_voxels * sizeof(float)));
            HIP_CHECK(hipMemset(d_Qi, 0, num_voxels * sizeof(float)));
    ...
    HIP_CHECK(hipFree(d_kx));
    HIP_CHECK(hipFree(d_ky));
    HIP_CHECK(hipFree(d_kz));
    HIP_CHECK(hipFree(d_phi_r));
    HIP_CHECK(hipFree(d_phi_i));
    HIP_CHECK(hipFree(d_Qr));
    HIP_CHECK(hipFree(d_Qi));
    free(h_x);
    free(h_y);
    free(h_z);
    free(h_kx);
