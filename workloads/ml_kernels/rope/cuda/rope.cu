// rope.cu -- CUDA Rotary Position Embedding (RoPE) benchmark
...
int main(int argc, char** argv) {
    ...
    CUDA_CHECK(cudaMemcpy(d_q, h_q, tensor_bytes, cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_k, h_k, tensor_bytes, cudaMemcpyHostToDevice));
    ...
    BenchResult r = runBenchmark("rope", problemSize, iterations, [&]() {
        rope_kernel<<<grid_size, block_size>>>(d_q, d_k,
