// activation.cpp -- HIP activation function benchmark (ReLU, GELU, SiLU)
...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < num_elements; i++) {
        h_input[i] = ((float)rand() / (float)RAND_MAX) * 10.0f - 5.0f; // range [-5, 5]
    }
    ...
    BenchResult r = runBenchmark(act_name, problemSize, iterations, [&]() {
        hipLaunchKernelGGL(activation_kernel, grid, block, 0, 0,
                           d_input, d_output, num_elements, act_type,
                           elements_per_thread);
    });
