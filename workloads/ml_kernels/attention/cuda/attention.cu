// attention.cu -- CUDA naive multi-head attention benchmark
...
int main(int argc, char** argv) {
    ...
    CUDA_CHECK(cudaMalloc(&d_output, qkv_bytes));
    ...
    BenchResult r = runBenchmark("naive_attention", problemSize, iterations, [&]() {
        qk_matmul_kernel<<<qk_grid, block_size>>>(d_Q, d_K, d_scores,
            batch_size, num_heads, seq_len, head_dim, scale);
        softmax_kernel<<<softmax_grid, block_size>>>(d_scores,
            softmax_total, seq_len);
        attn_v_matmul_kernel<<<av_grid, block_size>>>(d_scores, d_V, d_output,
