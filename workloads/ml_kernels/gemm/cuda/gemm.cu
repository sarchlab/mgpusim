// gemm.cu -- CUDA tiled shared-memory GEMM benchmark
...
int main(int argc, char** argv) {
    ...
    if (tile_size == 8) {
        ...
        r = runBenchmark("tiled_gemm_8", problemSize, iterations, [&]() {
            tiled_gemm<8><<<grid, block>>>(d_A, d_B, d_C, M, N, K,
                                           alpha, beta);
        });
        ...
        r = runBenchmark("tiled_gemm_16", problemSize, iterations, [&]() {
            tiled_gemm<16><<<grid, block>>>(d_A, d_B, d_C, M, N, K,
                                            alpha, beta);
        });
    } else {
        ...
        r = runBenchmark("tiled_gemm_32", problemSize, iterations, [&]() {
            tiled_gemm<32><<<grid, block>>>(d_A, d_B, d_C, M, N, K,
                                            alpha, beta);
        });
    }
