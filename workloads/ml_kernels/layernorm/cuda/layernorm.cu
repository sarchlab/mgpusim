// layernorm.cu -- CUDA LayerNorm / RMSNorm benchmark
...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < hidden_dim; i++) {
        ...
        h_beta[i]  = ((float)rand() / RAND_MAX) * 0.01f;
    }
    ...
    CUDA_CHECK(cudaMemcpy(d_gamma, h_gamma, param_bytes,
                          cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_beta, h_beta, param_bytes,
                          cudaMemcpyHostToDevice));
    ...
    BenchResult r = runBenchmark(norm_name, problemSize, iterations, [&]() {
        layernorm_kernel<<<grid, block>>>(d_input, d_gamma, d_beta,
                                          d_output, num_rows, hidden_dim,
                                          eps, norm_type);
    });
    ...
    // input: num_rows * hidden_dim, gamma: hidden_dim, beta: hidden_dim, output: num_rows * hidden_dim
