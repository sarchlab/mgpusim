// fused_swiglu.cu -- CUDA fused SwiGLU activation benchmark
...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < num_elements; i++) {
        h_gate[i] = ((float)rand() / RAND_MAX) * 6.0f - 3.0f; // range [-3, 3]
        h_up[i]   = ((float)rand() / RAND_MAX) * 6.0f - 3.0f; // range [-3, 3]
    }

    // Allocate device memory
    float *d_gate, *d_up, *d_output;
    CUDA_CHECK(cudaMalloc(&d_gate, data_bytes));
    CUDA_CHECK(cudaMalloc(&d_up, data_bytes));
    CUDA_CHECK(cudaMalloc(&d_output, data_bytes));

    CUDA_CHECK(cudaMemcpy(d_gate, h_gate, data_bytes, cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_up, h_up, data_bytes, cudaMemcpyHostToDevice));
    ...
    BenchResult r = runBenchmark("fused_swiglu", problemSize, iterations, [&]() {
        fused_swiglu_kernel<<<grid, block>>>(d_gate, d_up, d_output,
                                              num_elements,
                                              elements_per_thread,
                                              precision);
    });
