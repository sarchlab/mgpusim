...
CUDA host program `correlation.cu` for the PolyBench/GPU CORR benchmark. The GPU kernels are not copied: `correlation.cu` `#include`s the HIP kernel file of the AMD benchmark, which nvcc compiles unchanged through `../../include/hip/hip_runtime.h`.

| | |
| --- | --- |
| Kernel source | `amd/benchmarks/polybench/correlation/native/polybench_correlation.cpp` |
| Kernel version | commit `146a4c8c` (2026-06-17) on main |
| Kernel origin | Extracted from `sarchlab/gpu_benchmarks` tier2/polybench_correlation |
| Kernels | `mean_kernel`, `stddev_kernel`, `normalize_kernel`, `correlation_kernel` |
