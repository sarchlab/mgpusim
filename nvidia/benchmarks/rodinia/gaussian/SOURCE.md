...
CUDA host program `gaussian.cu` for the Rodinia Gaussian benchmark. The GPU kernels are not copied: `gaussian.cu` `#include`s the HIP kernel file of the AMD benchmark, which nvcc compiles unchanged through `../../include/hip/hip_runtime.h`.

| | |
| --- | --- |
| Kernel source | `amd/benchmarks/rodinia/gaussian/native/rodinia_gaussian.cpp` |
| Kernel version | commit `146a4c8c` (2026-06-17) on main |
| Kernel origin | Extracted from `sarchlab/gpu_benchmarks` tier2/rodinia_gaussian |
| Kernels | `fan1`, `fan2` |
| Host code follows | `amd/benchmarks/rodinia/gaussian/gaussian.go` |
| Host code version | commit `90b1b660` (2026-06-25) on main |
| CUDA port written | 2026-09-23 |
| Binary | `nvidia/benchmarks/bin/gaussian` |
