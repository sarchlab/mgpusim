...
CUDA host program `lud.cu` for the Rodinia LUD benchmark. The GPU kernels are not copied: `lud.cu` `#include`s the HIP kernel file of the AMD benchmark, which nvcc compiles unchanged through `../../include/hip/hip_runtime.h`.

| | |
| --- | --- |
| Kernel source | `amd/benchmarks/rodinia/lud/native/rodinia_lud.cpp` |
| Kernel version | commit `146a4c8c` (2026-06-17) on main |
| Kernel origin | Extracted from `sarchlab/gpu_benchmarks` tier2/rodinia_lud |
| Kernels | `lud_diagonal`, `lud_perimeter`, `lud_internal` |
| Host code follows | `amd/benchmarks/rodinia/lud/lud.go` |
| Host code version | commit `90b1b660` (2026-06-25) on main |
| CUDA port written | 2026-09-23 |
| Binary | `nvidia/benchmarks/bin/lud` |
