...
func (m *TraceExecMeta) ExecType() nvidiaconfig.ExecType {
	return m.execType
}
...
func (t *KernelTrace) readThreadblocks() {
	t.tbIDToIndex = make(map[nvidiaconfig.Dim3]int32)
...
func extractInst(text string) *Instruction {
	...
	for i := 0; i < int(inst.DestNum); i++ {
		inst.DestRegs = append(inst.DestRegs, nvidiaconfig.NewRegister(elems[2+i+1]))
	}
	...
	for i := 0; i < int(inst.SrcNum); i++ {
		inst.SrcRegs = append(inst.SrcRegs, nvidiaconfig.NewRegister(elems[4+int(inst.DestNum)+i+1]))
	}
