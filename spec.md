...
- Error formula: `err = (sim - hw) / min(sim, hw)` (symmetrical)

## Constraints

1. **SIMD width is 16 for CDNA3** — confirmed by ISA documentation (TRAP_STS.EXCP_CYCLE register). Do NOT set NumSinglePrecisionUnit to 32. PR #256 in upstream has this wrong and should be corrected.
...
- **VecMem pipeline**: Currently configurable (default: inst=6 stages, trans=10 stages; MI300A override: inst=2, trans=4)
