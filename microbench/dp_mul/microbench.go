...
func initMem() {
	dataSize = 32000
	gVector1 = gpuDriver.AllocateMemory(globalMem.Storage, uint64(dataSize*8))
	gVector2 = gpuDriver.AllocateMemory(globalMem.Storage, uint64(dataSize*8))
	gOutput =  gpuDriver.AllocateMemory(globalMem.Storage, uint64(dataSize*8))

	v1 := make([]float64, dataSize)
	v2 := make([]float64, dataSize)
