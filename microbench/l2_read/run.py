...
def generate_benchmark(count):
    ...
    for i in range(0, 4096):
        pre_scan_insts += 'flat_load_dword v0, v[1:2]\n'
        pre_scan_insts += 'v_add_u32 v1, vcc, v1, v3\n'
        pre_scan_insts += 'v_addc_u32 v2, vcc, v2, 0, vcc\n'
        pre_scan_insts += 's_mov_b64 vcc, 0\n'
        pre_scan_insts += 's_waitcnt vmcnt(0)\n'

    insts = ''
    for i in range(0, count):
        insts += 'flat_load_dword v0, v[4:5]\n'
        insts += 'v_add_u32 v4, vcc, v4, v3\n'
        insts += 'v_addc_u32 v5, vcc, v5, 0, vcc\n'
...
def run_on_gpu(num_access, repeat):
    ...
    for i in range(0, repeat):
        ...
        entry = ['l2_read', 'gpu', num_access, duration]
        print(entry)
        data = data.append(
            pd.DataFrame([entry], columns=data_columns),
            ignore_index=True,
