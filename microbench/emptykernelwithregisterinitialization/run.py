...
def run():
    ...
    for numWfPerWG in range(1, 9):
        ...
        for numWG in range(128, 1025, 128):
            print('numWG', numWG)
            time = run_benchmark('./emptykernelwithregisterinitialization -timing -numWfPerWG {0} -numWG {1}'.format(numWfPerWG, numWG), '.')
            data = data.append(
                pd.DataFrame([['empty_kernel_with_register_initialization', numWfPerWG, numWG, time]], columns=data_columns),
                ignore_index=True,
            )
