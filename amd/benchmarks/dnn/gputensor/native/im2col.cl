...
__kernel void im2col_2d(__global float *input, __global float *output,
                        const uint2 inputDimensions, const uint2 maskDimensions,
                        const uint2 stride, const uint2 pad,
                        const uint2 dilation, const uint channel,
                        const uint batch) {
  uint tid_x = get_global_id(0);
  uint tid_y = get_global_id(1);
