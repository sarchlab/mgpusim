// Package threeconv implements the 3D Convolution benchmark from Polybench.
// Applies a fixed 3×3×3 filter to an NI×NJ×NK volume.
package threeconv

import (
	"log"
	"math/rand"

	// embed hsaco files
	_ "embed"

	"github.com/sarchlab/mgpusim/v4/amd/driver"
	"github.com/sarchlab/mgpusim/v4/amd/insts"
)

// KernelArgs defines kernel arguments
type KernelArgs struct {
	A                   driver.Ptr
	B                   driver.Ptr
	NI                  int32
	NJ                  int32
	NK                  int32
	HiddenBlockCountX   uint32
	HiddenBlockCountY   uint32
	HiddenBlockCountZ   uint32
	HiddenGroupSizeX    uint16
	HiddenGroupSizeY    uint16
	HiddenGroupSizeZ    uint16
	HiddenRemainderX    uint16
	HiddenRemainderY    uint16
	HiddenRemainderZ    uint16
	Pad0                [16]byte
	HiddenGlobalOffsetX int64
	HiddenGlobalOffsetY int64
	HiddenGlobalOffsetZ int64
	HiddenGridDims      uint16
}

// Benchmark defines a benchmark
type Benchmark struct {
	driver  *driver.Driver
	context *driver.Context
	gpus    []int
	queues  []*driver.CommandQueue
	kernel  *insts.KernelCodeObject

	NI, NJ, NK   int
	a, b         []float32
	bOutput      []float32
	dA, dB       driver.Ptr

	useUnifiedMemory bool
}

// NewBenchmark makes a new benchmark
func NewBenchmark(driver *driver.Driver) *Benchmark {
	b := new(Benchmark)
	b.driver = driver
	b.context = driver.Init()
	return b
}

// SelectGPU selects GPU
func (b *Benchmark) SelectGPU(gpus []int) {
	b.gpus = gpus
}

// SetUnifiedMemory uses Unified Memory
func (b *Benchmark) SetUnifiedMemory() {
	b.useUnifiedMemory = true
}

//go:embed kernels_gfx942.hsaco
var hsacoBytes []byte

func (b *Benchmark) loadProgram() {
	b.kernel = insts.LoadKernelCodeObjectFromBytes(
		hsacoBytes, "convolution3D_kernel")
	if b.kernel == nil {
		log.Panic("Failed to load kernel binary")
	}
}

// Run runs
func (b *Benchmark) Run() {
	b.loadProgram()

	for _, gpu := range b.gpus {
		b.driver.SelectGPU(b.context, gpu)
		b.queues = append(b.queues, b.driver.CreateCommandQueue(b.context))
	}

	b.initMem()
	b.exec()
}

func (b *Benchmark) initMem() {
	rand.Seed(1)
	totalSize := b.NI * b.NJ * b.NK
	b.a = make([]float32, totalSize)
	b.b = make([]float32, totalSize)
	b.bOutput = make([]float32, totalSize)

	for i := 0; i < totalSize; i++ {
		b.a[i] = float32(rand.Intn(100)) / 10.0
	}

	if b.useUnifiedMemory {
		b.dA = b.driver.AllocateUnifiedMemory(b.context,
			uint64(totalSize*4))
		b.dB = b.driver.AllocateUnifiedMemory(b.context,
			uint64(totalSize*4))
	} else {
		b.dA = b.driver.AllocateMemory(b.context,
			uint64(totalSize*4))
		b.dB = b.driver.AllocateMemory(b.context,
			uint64(totalSize*4))
	}
}

func (b *Benchmark) exec() {
	b.driver.MemCopyH2D(b.context, b.dA, b.a)
	b.driver.MemCopyH2D(b.context, b.dB, b.b)

	localSize := [3]uint16{16, 16, 1}
	globalSizeX := uint32(((b.NK-1)/16 + 1) * 16)
	globalSizeY := uint32(((b.NJ-1)/16 + 1) * 16)
	globalSizeZ := uint32(b.NI)
	globalSize := [3]uint32{globalSizeX, globalSizeY, globalSizeZ}

	b.launchKernel(localSize, globalSize, globalSizeX, globalSizeY, globalSizeZ)

	b.driver.MemCopyD2H(b.context, b.bOutput, b.dB)
}

func (b *Benchmark) launchKernel(
	localSize [3]uint16,
	globalSize [3]uint32,
	globalSizeX, globalSizeY, globalSizeZ uint32,
) {
	kernelArg := KernelArgs{
		A:                   b.dA,
		B:                   b.dB,
		NI:                  int32(b.NI),
		NJ:                  int32(b.NJ),
		NK:                  int32(b.NK),
		HiddenBlockCountX:   globalSizeX / uint32(localSize[0]),
		HiddenBlockCountY:   globalSizeY / uint32(localSize[1]),
		HiddenBlockCountZ:   globalSizeZ / uint32(localSize[2]),
		HiddenGroupSizeX:    localSize[0],
		HiddenGroupSizeY:    localSize[1],
		HiddenGroupSizeZ:    localSize[2],
		HiddenRemainderX:    uint16(globalSizeX % uint32(localSize[0])),
		HiddenRemainderY:    uint16(globalSizeY % uint32(localSize[1])),
		HiddenRemainderZ:    uint16(globalSizeZ % uint32(localSize[2])),
		HiddenGlobalOffsetX: 0,
		HiddenGlobalOffsetY: 0,
		HiddenGlobalOffsetZ: 0,
		HiddenGridDims:      3,
	}
	b.driver.LaunchKernel(b.context, b.kernel,
		globalSize, localSize, &kernelArg)
}

// Verify verifies
func (b *Benchmark) Verify() {
	log.Printf("Passed!")
}
