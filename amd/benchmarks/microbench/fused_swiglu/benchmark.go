// Package fusedswiglu implements fused SwiGLU activation microbenchmark.
package fusedswiglu
...
// KernelArgs defines kernel arguments for fused_swiglu_kernel.
...
// Benchmark defines the fused SwiGLU benchmark.
...
// NewBenchmark creates a new fused SwiGLU benchmark.
func NewBenchmark(driver *driver.Driver) *Benchmark {
	b := new(Benchmark)
	b.driver = driver
	b.context = driver.Init()
	b.NumElements = 1572864
	b.BlockSize = 256
	return b
}
...
func (b *Benchmark) loadProgram() {
	b.kernel = insts.LoadKernelCodeObjectFromBytes(
		hsacoBytes, "fused_swiglu_kernel")
	if b.kernel == nil {
		log.Panic("Failed to load fused_swiglu_kernel")
	}
}
...
func (b *Benchmark) initMem() {
	...
	b.hA = make([]float32, n)
	b.hGate = make([]float32, n)
	b.hB = make([]float32, n)

	for i := 0; i < n; i++ {
		b.hA[i] = float32(i%1000)*0.002 - 1.0
		b.hGate[i] = float32(i%500)*0.004 - 1.0
	}
...
func (b *Benchmark) Verify() {
	...
	if errors > 0 {
		log.Panicf("FAIL: %d errors in fused_swiglu verification", errors)
	}
	log.Printf("Passed!")
}
