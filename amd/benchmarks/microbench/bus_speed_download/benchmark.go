// Package busspeeddownload implements bus speed download (H2D) microbenchmark.
package busspeeddownload
...
// KernelArgs defines kernel arguments for bus_speed_download_kernel.
...
// Benchmark defines the bus speed download benchmark.
...
// NewBenchmark creates a new bus speed download benchmark.
...
func (b *Benchmark) loadProgram() {
	b.kernel = insts.LoadKernelCodeObjectFromBytes(
		hsacoBytes, "bus_speed_download_kernel")
	if b.kernel == nil {
		log.Panic("Failed to load bus_speed_download_kernel")
	}
}
...
func (b *Benchmark) Verify() {
	...
	if errors > 0 {
		log.Panicf("FAIL: %d errors in bus_speed_download verification",
			errors)
	}
	log.Printf("Passed!")
}
