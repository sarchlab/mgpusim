// Package devicememoryread implements device memory read bandwidth microbenchmark.
package devicememoryread
...
// KernelArgs defines kernel arguments for device_memory_read_kernel.
...
// Benchmark defines the device memory read benchmark.
...
// NewBenchmark creates a new device memory read benchmark.
...
func (b *Benchmark) loadProgram() {
	b.kernel = insts.LoadKernelCodeObjectFromBytes(
		hsacoBytes, "device_memory_read_kernel")
	if b.kernel == nil {
		log.Panic("Failed to load device_memory_read_kernel")
	}
}
...
func (b *Benchmark) Verify() {
	...
	if errors > 0 {
		log.Panicf("FAIL: %d errors in device_memory_read verification",
			errors)
	}
	log.Printf("Passed!")
}
