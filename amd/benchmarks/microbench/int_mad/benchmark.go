// Package intmad implements integer multiply-add throughput microbenchmark.
package intmad
...
// KernelArgs defines kernel arguments for int_mad_kernel.
...
// Benchmark defines the integer MAD benchmark.
...
// NewBenchmark creates a new integer MAD benchmark.
...
func (b *Benchmark) loadProgram() {
	b.kernel = insts.LoadKernelCodeObjectFromBytes(
		hsacoBytes, "int_mad_kernel")
	if b.kernel == nil {
		log.Panic("Failed to load int_mad_kernel")
	}
}
...
func (b *Benchmark) initMem() {
	numThreads := 65536
	bytes := uint64(numThreads * 4) // 4 bytes per int32
