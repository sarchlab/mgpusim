// Package occupancyfma implements occupancy FMA microbenchmark with high register pressure.
package occupancyfma
...
// KernelArgs defines kernel arguments for occupancy_fma_kernel.
...
// Benchmark defines the occupancy FMA benchmark.
...
// NewBenchmark creates a new occupancy FMA benchmark.
...
func (b *Benchmark) loadProgram() {
	b.kernel = insts.LoadKernelCodeObjectFromBytes(
		hsacoBytes, "occupancy_fma_kernel")
	if b.kernel == nil {
		log.Panic("Failed to load occupancy_fma_kernel")
	}
}
...
// Verify verifies the result — just checks output is non-zero (complex to verify exactly).
func (b *Benchmark) Verify() {
	...
	if nonZero == 0 {
		log.Panicf("FAIL: all outputs are zero in occupancy_fma")
	}
	log.Printf("Passed! (%d/%d non-zero outputs)", nonZero, n)
}
