...
var _ = Describe("LDS Unit", func() {
	...
	It("should run", func() {
		...
		// wave2: ALU runs, cycleLeft set to 4, stays in toExec
		...
		// Run 4 more cycles to drain cycleLeft for wave2
		for i := 0; i < 3; i++ {
			bu.Run()
		}

		// After 3 more runs, cycleLeft should be 1, wave2 still in toExec
		Expect(bu.toExec).To(BeIdenticalTo(wave2))
		Expect(bu.toWrite).To(BeNil())

		// 4th run: cycleLeft reaches 0, wave2 moves to toWrite
		bu.Run()
