...
// on its execution unit. The isCDNA3 flag selects CDNA3-specific latencies
// (e.g. full-rate FP64). LDS/VMem instructions return 0 (not tracked by
// scoreboard; handled by s_waitcnt).
func GetScoreboardLatency(inst *insts.Inst, isCDNA3 bool) int {
	switch inst.ExeUnit {
	case insts.ExeUnitVALU:
		if isTranscendental(inst.InstName) {
			return 32
		}
		if strings.Contains(inst.InstName, "f64") {
			if isCDNA3 {
				return 8 // CDNA3: full-rate FP64 (pipeline=4 + writeback×2 for 64-bit)
			}
			return 16
		}
		return 4 // pipeline=4 cycles
