...
var _ = Describe("Scoreboard", func() {
	...
	Describe("GetScoreboardLatency", func() {
		It("should return 4 for VALU FP32", func() {
			...
			Expect(GetScoreboardLatency(inst, false)).To(Equal(4))
		})

		It("should return 16 for VALU FP64 (non-CDNA3)", func() {
			inst := insts.NewInst()
			inst.ExeUnit = insts.ExeUnitVALU
			inst.InstName = "v_add_f64"

			Expect(GetScoreboardLatency(inst, false)).To(Equal(16))
		})

		It("should return 8 for VALU FP64 on CDNA3 (full-rate)", func() {
			inst := insts.NewInst()
			inst.ExeUnit = insts.ExeUnitVALU
			inst.InstName = "v_add_f64"

			Expect(GetScoreboardLatency(inst, true)).To(Equal(8))
		})

		It("should return 4 for VALU integer", func() {
			inst := insts.NewInst()
			inst.ExeUnit = insts.ExeUnitVALU
			inst.InstName = "v_add_u32"

			Expect(GetScoreboardLatency(inst, false)).To(Equal(4))
		})

		It("should return 32 for VALU transcendental", func() {
			inst := insts.NewInst()
			inst.ExeUnit = insts.ExeUnitVALU
			inst.InstName = "v_rcp_f32"

			Expect(GetScoreboardLatency(inst, false)).To(Equal(32))
		})

		It("should return 32 for VALU FP64 transcendental", func() {
			inst := insts.NewInst()
			inst.ExeUnit = insts.ExeUnitVALU
			inst.InstName = "v_rcp_f64"
		...
		It("should return 2 for scalar", func() {
			...
			Expect(GetScoreboardLatency(inst, false)).To(Equal(2))
		})
		...
		It("should return 3 for branch", func() {
			...
			Expect(GetScoreboardLatency(inst, false)).To(Equal(3))
		})
		...
		It("should return 0 for VMem", func() {
			...
			Expect(GetScoreboardLatency(inst, false)).To(Equal(0))
		})
