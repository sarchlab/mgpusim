...
// ctrlMiddleware implements the memcontrolprotocol universal verbs
// (Pause/Drain/Enable/Reset) on the "Control" port, mirroring akita v5's
// mem/simplebankedmemory control middleware.
//
// The v4 mgpusim simplebankedmemory had no control port; the protocol is new
// in v5. The command processor's v4 flush/restart flow maps onto the verbs as
// CmdPause -> CmdReset for "flush and discard", and CmdEnable for "restart".
...
// completePendingDrain finalizes a Drain when no work is in flight. The
// component is quiescent when the pending queue is empty and every bank's
// pipeline, post-pipeline buffer, and row-miss delay queue are empty.
...
func bankIsQuiescent(b *bankState) bool {
	return len(b.Pipeline.Stages()) == 0 &&
...
// discards the pending queue, and clears the Top port queue.
