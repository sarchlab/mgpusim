...
type Builder struct {
	...
	rowHitDelay        int    // extra cycles for row hit (0 = disabled)
}
...
// WithRowHitDelay sets the extra delay in cycles for a row buffer hit.
// When set to 0 (default), row hits go directly into the pipeline.
func (b Builder) WithRowHitDelay(cycles int) Builder {
	b.rowHitDelay = cycles
	return b
}
...
func (b Builder) Build(name string) *Comp {
	...
	for i := range c.banks {
		postPipelineBuf := queueing.Buffer[*bankPipelineItem]{
			BufferName: fmt.Sprintf("%s.Bank[%d].PostPipelineBuffer", name, i),
			Cap:        b.postPipelineBufSize,
		}

		pipeline := queueing.Pipeline[*bankPipelineItem]{
			Width:     b.bankPipelineWidth,
			NumStages: b.bankPipelineDepth,
		}
