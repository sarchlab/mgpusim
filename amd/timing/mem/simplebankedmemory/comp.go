...
func (i *bankPipelineItem) TaskID() string {
	return fmt.Sprintf("%d_pl", i.req.Meta().ID)
}
	...
	for _, req := range m.pendingReqs {
		...
		if m.rowBufferSizeLog2 > 0 && m.rowMissDelay > 0 {
			...
			if b.rowValid && b.lastRowAddr == rowAddr {
				// ROW HIT
				if m.rowHitDelay > 0 {
					// Add to delay queue with rowHitDelay
					b.delayQueue = append(b.delayQueue, delayedItem{
						item:       item,
						cyclesLeft: m.rowHitDelay,
					})
				} else {
					// Accept into pipeline directly
...
func (m *middleware) finalizeSingle(b *bank) bool {
	if b.postPipelineBuf.Size() == 0 {
		return false
	}

	item := b.postPipelineBuf.Elements[0]
...
func (m *middleware) finalizeRead(
	...
	// Pop from buffer
	b.postPipelineBuf.Elements = b.postPipelineBuf.Elements[1:]
...
func (m *middleware) tickPipelines() bool {
	...
	for i := range m.banks {
		...
		madeProgress = p.Tick(&m.banks[i].postPipelineBuf) || madeProgress
	}
