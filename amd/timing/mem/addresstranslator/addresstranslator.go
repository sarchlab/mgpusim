...
func (m *middleware) translate() bool {
	...
	transReq := &vm.TranslationReq{
		MsgMeta: sim.MsgMeta{
			...
			Dst: m.translationPortMapper.Find(vAddr),
		},
		PID:      req.GetPID(),
		VAddr:    vPageID,
		DeviceID: m.deviceID,
...
func (m *middleware) parseTranslation() bool {
	...
	transaction := m.findTranslationByReqID(transRsp.RspTo)
...
func (m *middleware) respond() bool {
	...
	if reqInBottom {
		...
		m.removeReqToBottomByID(rsp.(mem.AccessRsp).Meta().RspTo)
...
func (m *middleware) createTranslatedReadReq(
	...
	clone := &mem.ReadReq{
		MsgMeta: sim.MsgMeta{
			...
			Dst: m.memoryPortMapper.Find(addr),
		},
		Address:            addr,
		AccessByteSize:     req.AccessByteSize,
		PID:                0,
		Info:               req.Info,
		CanWaitForCoalesce: req.CanWaitForCoalesce,
	}
...
func (m *middleware) findTranslationByReqID(id uint64) *transaction {
	for _, t := range m.transactions {
...
func (m *middleware) isReqInBottomByID(id uint64) bool {
...
func (m *middleware) findReqToBottomByID(id uint64) reqToBottom {
	for _, r := range m.inflightReqToBottom {
		if r.reqToBottom.Meta().ID == id {
			return r
		}
...
func (m *middleware) removeReqToBottomByID(id uint64) {
